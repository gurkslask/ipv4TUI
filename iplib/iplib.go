...
func (ip *IPv4) SetAddress(in string) error {
	...
	for i := 0; i < 4; i++ {
		err := ip.octets[i].Setnum(common.StrToInt(splittedoctets[i]))
		if err != nil {
			return err
		}
	}
...
func (ip IPv4) GetBits() [32]bool {
	...
	for octetnum, octetbyte := range b {
		bb := common.GetBoolsFromByte(octetbyte)
...
func CalcBroadcastAddress(ip IPv4, subnetmask IPv4) IPv4 {
	...
	for i := range 4 {
		bb[i] = common.GetByteFromBools(res[i*8 : i*8+8])
	}
	resIP, _ := NewIPv4FromBinary(bb)
	return resIP
}
